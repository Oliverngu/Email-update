...
import { ReservationSetting, ThemeSettings, GuestFormSettings } from '../../../core/models/data';
...
export const ReservationSettingsModal: FC<ReservationSettingsModalProps> = ({ unitId, onClose }) => {
...
const ThemePreview: FC<{theme: ThemeSettings}> = ({ theme }) => {
     ...
     return (
        <div style={{ backgroundColor: theme.background, color: theme.textPrimary }} className={`p-4 ${radiusClass} ${fontBaseClass} h-full`}>
            <div style={{ backgroundColor: theme.surface }} className={`p-4 ${radiusClass} ${shadowClass}`}>
                <h3 className="font-bold text-lg" style={{ color: theme.textPrimary }}>Ez egy előnézet</h3>
                <p className="mt-1" style={{ color: theme.textSecondary }}>Itt láthatod, hogyan fognak kinézni az elemek.</p>
                <button style={{ backgroundColor: theme.primary, color: '#ffffff' }} className={`mt-4 w-full p-2 ${radiusClass} font-semibold`}>Gomb</button>
            </div>
        </div>
    );
};
